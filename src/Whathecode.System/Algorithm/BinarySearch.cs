...
namespace Whathecode.System.Algorithm
{
		...
		where TIndex : IComparable<TIndex>
	{
			...
			IndexerDelegates<TObject, TIndex> indexOperations )
		{
			...
			if ( range.Start.CompareTo( indexOperations.GetNearestIndex( range.Start ) ) != 0 ||
				 range.End.CompareTo( indexOperations.GetNearestIndex( range.End ) ) != 0 )
			{
				throw new ArgumentException( "The start and end of the range should be valid indices.", nameof( range ) );
			}
