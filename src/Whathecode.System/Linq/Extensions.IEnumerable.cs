...
namespace Whathecode.System.Linq
{
	...
	public static partial class Extensions
	{
		...
		public static IEnumerable<T> TakeRandom<T>( this IEnumerable<T> source, int count, Random random )
		{
			...
			{
				throw new ArgumentException( "Count can not be greater than amount of items available in the source.", nameof( count ) );
			}
		...
		public static bool CountOf<T>( this IEnumerable<T> source, int count )
		{
			...
			if ( count < 0 )
			{
				throw new ArgumentException( "Count needs to be bigger than zero.", nameof( count ) );
			}
