...
namespace Whathecode.Tests.System.Arithmetic.Range
{
	...
	public class IntervalTest
	{
			...
			where TSize : IComparable<TSize>
		{
			...
			Assert.Equal( minValue, empty.Start );
			Assert.False( empty.IsStartIncluded );
			Assert.Equal( minValue, empty.End );
			Assert.False( empty.IsEndIncluded );
			Assert.Equal( size, empty.Size );

			// Test that reference stays the same.
			var secondEmpty = Interval<T, TSize>.Empty;
			Assert.Same( empty, secondEmpty );
		}
		...
		public void IntersectsTest()
		{
			...
			Assert.True( Interval<double, double>.Empty.Intersects( new Interval<double, double>( -10, true, 10, true ) ) );
			Assert.True( new Interval<double, double>( -10, true, 10, true ).Intersects( Interval<double, double>.Empty ) );
		}
		...
		public void SubtractTest()
		{
			...
			Assert.Equal( interval, result[ 0 ] );
		}
		...
		public void ScaleTest()
		{
			...
			Assert.Equal( Interval<int, int>.Empty.Scale( 2, 0 ), Interval<int, int>.Empty );
